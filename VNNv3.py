...
class VNNv3 (nn.Module):
    def __init__(self, d_model, input_kernel_size, output_kernel_size, device) -> None:
        super().__init__()

        self.weight_nn = nn.Sequential(
            nn.Linear(d_model*2, 12),
            nn.Tanh(),
            nn.Linear(12, input_kernel_size*output_kernel_size),
        )

        self.bias_nn = nn.Sequential(
            nn.Linear(d_model, 12),
            nn.Tanh(),
            nn.Linear(12, input_kernel_size),
        ) 
    ...
    def forward (self, x:torch.Tensor, output_size:int, debug=False):
        ...
        weight = torch.tanh(weight[:input_size, :output_size])
        ...
        if debug: 
            return out + bias_out, gen_x, gen_y
        else:  
            return out + bias_out
